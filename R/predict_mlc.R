...
#' @param ... other arguments (not relevant in this case)
#'
...
predict.mlc <- function(object, newdata, ...) {
  if (inherits(newdata, "SpatRaster")) {
    ...
    cli::cli_abort("newdata must be data frame or SpatRaster")
  }
  class_predict <- purrr::map_int(1:nrow(newdata_df),
    .progress = TRUE,
